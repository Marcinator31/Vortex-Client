...
public final class RadarRenderer {
    ...
    private static void renderInner(GuiGraphics context, Minecraft client) {
     ...
     */
    private static boolean drawMobEgg(GuiGraphics context,
        ...
        try {
            ...
            if (eggHolder == null) return false;
            var egg = eggHolder;
            ...
            var m = context.pose();
            m.pushPose();
            m.translate(ix, iy, 0.0D);
            m.scale(factor, factor, 1.0F);
            context.renderItem(EGG_STACKS.computeIfAbsent(egg,
    ...
    private static void drawPlayerInfo(GuiGraphics context, Minecraft client,
        ...
        try {
            ...
            if (handler != null) {
                ...
                if (entry != null) {
                    var skin = entry.getSkinLocation();
                    // Kopf zeichnen (inkl. Hut-Overlay).
                    context.blit(skin, hx, hy, 8.0F, 8.0F,
                            headSize, headSize, 64, 64);
                    context.blit(skin, hx, hy, 40.0F, 8.0F,
                            headSize, headSize, 64, 64);
                }
            }
            ...
            var m = context.pose();
            m.pushPose();
            m.translate(textX, textY, 0.0D);
            m.scale(textScale, textScale, 1.0F);
            context.drawString(client.font, Component.literal(info),
                    0, 0, 0xFFFFFFFF);
            m.popPose();
        } catch (Throwable ignored) {
    ...
    private static void drawCircle(GuiGraphics context, int cx, int cy, int r, int color) {
        if (r <= 0) return;
     ...
     * farblich auf.
     */
    private static void drawPlayerArrow(GuiGraphics context, int cx, int cy, int size) {
    ...
    private static void fillTriangleUp(GuiGraphics context, int cx, int cy, int size, int color) {
        int apexY = cy - size;
        int baseY = cy + size;
