...
public final class WaypointHud {
    ...
    public static void draw(GuiGraphics ctx, Minecraft client) {
        ...
        if (com.vortex.client.waypoint.WaypointActions.hasPendingCorner()) {
            ...
            ctx.drawString(client.font, AREA_HINT,
            ...
            if (grp != null) {
                ...
                ctx.drawString(client.font, groupHint,
        ...
        if (mod.showNearest.get()) {
            ...
            if (near != null) {
                ...
                ctx.drawString(client.font, nearestText,
        ...
        for (WaypointManager.Waypoint wp : visibleHere) {
            ...
            if (s.visible) {
                ...
                if (mod.showLetter.get()) {
                    ...
                    ctx.drawString(client.font, memo.letter,
                ...
                if (a > 0.02f && mod.labels.get()) {
                    ...
                    ctx.drawString(client.font, memo.label,
     ...
     */
    private static void drawRing(GuiGraphics ctx, int cx, int cy, int r,
    ...
    private static void pushScale(GuiGraphics ctx, float ax, float ay, float scale) {
        var m = ctx.pose();
        m.pushPose();
        m.translate(ax, ay, 0.0D);
        m.scale(scale, scale, 1.0F);
        m.translate(-ax, -ay, 0.0D);
    }

    private static void popScale(GuiGraphics ctx) {
        ctx.pose().popPose();
    }
     ...
     */
    private static void drawEdgeArrow(GuiGraphics ctx, Minecraft client,
        ...
        ctx.drawString(client.font, txt, tx, ty, color);
    }
}
