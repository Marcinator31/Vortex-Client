...
public class SkinScreen extends Screen {
    ...
    private void drawHeader(GuiGraphics ctx, int accent) {
        ...
        ctx.drawString(this.font, Component.literal("Skin Wardrobe"),
        ...
        if (searchField != null && searchField.getValue().isEmpty()) {
            ctx.drawString(this.font, Component.literal("Enter a player name..."),
        ...
        ctx.drawString(this.font, Component.literal(ol), ox + 9, winY + 40, 0xFFD0D0DA, false);
        ...
        ctx.drawString(this.font, Component.literal(il), ix + 9, winY + 40, 0xFFD0D0DA, false);
        hits.add(new Hit(ix, winY + 34, iw, 20, Act.IMPORT, null));
    }

    private void drawGrid(GuiGraphics ctx, int accent) {
        ...
        for (SkinWardrobe.Skin skin : list) {
            ...
            ctx.drawString(this.font, Component.literal(src),
            ...
            if (hov) {
                ...
                ctx.drawString(this.font, Component.literal("rename"),
                ...
                ctx.drawString(this.font, Component.literal(ml), mxp, by, 0xFFB4B4C0, false);
            ...
            ctx.drawString(this.font, Component.literal(upShort),
                    ubX + (ubW - uw2) / 2, ubY + 3,
     ...
     */
    private void drawSkinPreview(GuiGraphics ctx, SkinWardrobe.Skin skin,
                                 int x, int y, int scale) {
        ResourceLocation tex = SkinTextureCache.get(skin);
        int size = 8 * scale;
        if (tex == null) {
        ...
        try {
            ...
            ctx.blit(tex, x, y, 8.0F, 8.0F, size, size, 64, 64);
            // Hut-Ebene darueber
            ctx.blit(tex, x, y, 40.0F, 8.0F, size, size, 64, 64);
    ...
    private void drawFooter(GuiGraphics ctx, int accent) {
        ...
        if (renaming != null) {
            ctx.drawString(this.font, Component.literal("New name:"),
        ...
        if (actNow != null) {
            ...
            ctx.drawString(this.font, Component.literal(vl),
        ...
        ctx.drawString(this.font, Component.literal(shorten(msg, winW - 24)),
