...
public class MacroScreen extends Screen {
    ...
    private void drawHeader(GuiGraphics ctx, int accent) {
     ...
     */
    private void wrapButton(GuiGraphics ctx, int[] pos, int right, String label,
    ...
    private int button(GuiGraphics ctx, int x, int y, String label, Act act,
    ...
    private void drawList(GuiGraphics ctx, int accent) {
        int x = winX + 8;
        ...
        if (MacroManager.all().isEmpty()) {
            ctx.drawString(this.font, Component.literal("No macros yet"),
        ...
        for (Macro m : MacroManager.all()) {
            ...
            ctx.drawString(this.font, Component.literal(label), x + 8, y + 3, 0xFFFFFFFF, false);
            ctx.drawString(this.font, Component.literal(m.steps.size() + " steps"),
    ...
    private void drawSteps(GuiGraphics ctx, int accent) {
        int x = winX + listW + 12;
        int w = winW - listW - 24;
        int top = winY + headerH;

        if (selected == null) {
            ctx.drawString(this.font, Component.literal("Pick a macro on the left"),
        ...
        for (Macro.Step step : new ArrayList<>(selected.steps)) {
            if (y + ROW_H >= top + 30 && y <= top + listH) {
                ...
                ctx.drawString(this.font, Component.literal(String.valueOf(i)),
                        x + 7, y + 7, 0xFF5A5A66, false);
                ctx.drawString(this.font, Component.literal(step.describe()),
                ...
                ctx.drawString(this.font, Component.literal(d), dx, y + 7, 0xFFD0D0DA, false);
                ...
                if (step.action == Macro.Action.KEY) {
                    ...
                    ctx.drawString(this.font, Component.literal(h), hx, y + 7, 0xFFD0D0DA, false);
                    hits.add(new Hit(hx - 5, y + 4, hw + 10, 14, Act.STEP_HOLD, step));
                }

                ctx.drawString(this.font, Component.literal("^"), x + w - 44, y + 7, 0xFF9A9AA6, false);
                ctx.drawString(this.font, Component.literal("v"), x + w - 30, y + 7, 0xFF9A9AA6, false);
                ctx.drawString(this.font, Component.literal("x"), x + w - 16, y + 7, 0xFFFF7A7A, false);
    ...
    private void drawFooter(GuiGraphics ctx, int accent) {
        ...
        if (editingSomething) {
            ...
            ctx.drawString(this.font, Component.literal(ok), oxx + 7, fy + 8, 0xFFFFFFFF, false);
        ...
        String hint = status.isEmpty()
                ...
                : status;
        ctx.drawString(this.font, Component.literal(shorten(hint, winW - 24)),
