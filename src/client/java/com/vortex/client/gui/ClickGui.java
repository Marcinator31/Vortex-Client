...
public class ClickGui extends Screen {
    ...
    private void drawHeader(GuiGraphics ctx, int x, int y, int w, int accent, Theme t) {
        ...
        ctx.drawString(this.font, Component.literal(Branding.title()),
        ...
        ctx.drawString(this.font, Component.literal(active + " active"),
        ...
        if (presetInfo != null && presetInfoTime < 3f) {
            ...
            ctx.drawString(this.font, Component.literal(presetInfo),
        ...
        ctx.drawString(this.font, Component.literal(design),
        ...
        if (search != null) {
            ...
            ctx.drawString(this.font, Component.literal("Q"),
    ...
    private void drawSidebar(GuiGraphics ctx, int x, int y, int h,
        ...
        if (GuiState.hasFavorites()) {
            ...
            ctx.drawString(this.font, Component.literal("* Favourites"),
                    x + 16, cy + 7,
            ...
            ctx.drawString(this.font, Component.literal(badge),
        ...
        for (Module.Category cat : Module.Category.values()) {
            ...
            ctx.drawString(this.font, Component.literal(pretty(cat.name())),
    ...
    private int drawSectionEntry(GuiGraphics ctx, int x, int cy, String label,
        ...
        ctx.drawString(this.font, Component.literal(label), x + 16, cy + 7,
                fade(isSel ? t.text.get() : t.textDim.get(), openAnim), false);
        if (badge != null) {
            int bw = this.font.width(badge);
            ctx.drawString(this.font, Component.literal(badge),
    ...
    private void drawContent(GuiGraphics ctx, int x, int y, int w, int h,
                             int accent, Theme t, float dt) {
        ...
        for (Module m : list) {
            ...
            if (visible) {
                ...
                ctx.drawString(this.font, Component.literal(m.getName()),
                        cx + 12, cy + 9, m.isEnabled() ? t.text.get() : 0xFFA8A8B4);
                if (hasContent(m)) {
                    ctx.drawString(this.font, Component.literal(ex > 0.5f ? "-" : "+"),
     ...
     */
    private void drawSectionContent(GuiGraphics ctx, int x, int y, int w, int h,
                                    int accent, Theme t) {
        int cx = x + PAD;
        int cw = w - PAD * 2 - 4;
        ...
        switch (section) {
            case WAYPOINTS: {
                ...
                ctx.drawString(this.font, Component.literal("Waypoints"),
                        cx, cy, t.text.get());
                ...
                ctx.drawString(this.font, Component.literal("Manage markers"),
            ...
            case MACROS: {
                ctx.drawString(this.font, Component.literal("Macros"),
                        cx, cy, t.text.get());
                ctx.drawString(this.font,
                ...
                ctx.drawString(this.font, Component.literal("Open macro editor"),
                        cx + 10, cy + 6, t.text.get(), false);
                ctx.drawString(this.font, Component.literal(">"),
                        cx + cw - 14, cy + 6, accent, false);
            ...
            case COMMUNITY: {
                ctx.drawString(this.font, Component.literal("Community"),
                        cx, cy, t.text.get());
                ctx.drawString(this.font,
                ...
                ctx.drawString(this.font, Component.literal("Browse shared macros"),
                        cx + 10, cy + 6, t.text.get(), false);
                ctx.drawString(this.font, Component.literal(">"),
                        cx + cw - 14, cy + 6, accent, false);
            ...
            case KEYS: {
                ...
                ctx.drawString(this.font, Component.literal("Open key list"),
                        cx + 10, cy + 6, t.text.get(), false);
                ctx.drawString(this.font, Component.literal(">"),
                        cx + cw - 14, cy + 6, accent, false);
            ...
            case SKINS: {
                ctx.drawString(this.font, Component.literal("Skins"),
                        cx, cy, t.text.get());
                ctx.drawString(this.font,
                ...
                ctx.drawString(this.font, Component.literal("Open skin wardrobe"),
                        cx + 10, cy + 6, t.text.get(), false);
                ctx.drawString(this.font, Component.literal(">"),
                        cx + cw - 14, cy + 6, accent, false);
            ...
            case DESIGN: {
                ctx.drawString(this.font, Component.literal("Theme"),
                        cx, cy, t.text.get());
                ctx.drawString(this.font,
                ...
                boolean hov = inRect(mx, my, cx, cy, cw, 20);
                roundRect(ctx, cx, cy, cw, 20, hov ? mix(C_INNER, accent, 0.35f) : C_INNER);
                ctx.drawString(this.font, Component.literal("Open theme editor"),
                        cx + 10, cy + 6, t.text.get(), false);
                ctx.drawString(this.font, Component.literal(">"),
                        cx + cw - 14, cy + 6, accent, false);
    ...
    private int drawSubButton(GuiGraphics ctx, Module m, int cx, int sy, int cw,
                              int accent, Theme t) {
        String label;
        Act act;
    ...
    private void drawSetting(GuiGraphics ctx, Module m, Setting s,
                             int x, int y, int w, int accent, Theme t) {
        String name = s.getName();

        if (s instanceof BooleanSetting b) {
            ctx.drawString(this.font, Component.literal(name), x, y + 6,
            ...
            ctx.drawString(this.font, Component.literal(name), x, y + 1,
                    t.textDim.get(), false);
            String val = fmt(n.get());
            int vw = this.font.width(val);
            ctx.drawString(this.font, Component.literal(val), x + w - vw, y + 1, accent, false);
            ...
            ctx.drawString(this.font, Component.literal(val),
    ...
    private void drawTooltip(GuiGraphics ctx, String text, int accent) {
        int maxW = 210;
        ...
        int ly = ty + 5;
        for (String l : lines) {
            ctx.drawString(this.font, Component.literal(l), tx + 6, ly, 0xFFD0D0DA, false);
            ly += 10;
        }
    }

    private void drawSwitch(GuiGraphics ctx, int x, int y, float on, int accent) {
        roundRect(ctx, x, y, 22, 11, mix(0xFF43434F, accent, on));
        int kx = x + 2 + (int) (on * 10f);
        ctx.fill(kx, y + 2, kx + 8, y + 9, 0xFFFFFFFF);
    }

    private void drawFooter(GuiGraphics ctx, int x, int y, int w) {
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double delta) {
        ...
        if (mx >= lastWinX && mx < lastWinX + SIDEBAR_W) {
            sideScrollTarget -= (float) delta * 28f;
     ...
     */
    private static void drawRingMark(GuiGraphics ctx, int cx, int cy, int r, int color) {
    ...
    private void pvpclient$captureKeyIfListening() {
        ...
        if (com.mojang.blaze3d.platform.InputConstants.isKeyDown(
                mc.getWindow().getWindow(), org.lwjgl.glfw.GLFW.GLFW_KEY_ESCAPE)) {
        ...
        for (int code = org.lwjgl.glfw.GLFW.GLFW_KEY_SPACE;
            ...
            if (com.mojang.blaze3d.platform.InputConstants.isKeyDown(mc.getWindow().getWindow(), code)) {
