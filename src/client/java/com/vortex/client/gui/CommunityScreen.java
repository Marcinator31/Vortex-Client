...
public class CommunityScreen extends Screen {
    ...
    public void render(GuiGraphics ctx, int mouseX, int mouseY, float delta) {
        ...
        for (Entry e : list) {
            if (y + ROW_H >= winY + HEADER_H && y <= winY + HEADER_H + listH) {
                ...
                ctx.drawString(this.font, Component.literal(e.kind().toUpperCase()),
                        winX + 16, y + 5, tagColor, false);
                ctx.drawString(this.font, Component.literal(e.name()),
                ...
                ctx.drawString(this.font, Component.literal(shorten(sub, winW - 160)),
                ...
                ctx.drawString(this.font, Component.literal(get),
        ...
        if (pendingPreset != null) {
            ...
            roundRect(ctx, bxx + bw - cw2 - 12, byy + 70, cw2, 18,
                    cHov ? C_HOV : C_INNER);
            ctx.drawString(this.font, Component.literal(cancel),
                    bxx + bw - cw2 - 4, byy + 75, 0xFF9A9AA6, false);

            ctx.drawString(this.font, Component.literal("The one in colour is active now"),
    ...
    private int tab(GuiGraphics ctx, int x, int y, String label, String value, int accent) {
        ...
        ctx.drawString(this.font, Component.literal(label), x + 8, y + 5, 0xFFE6E6EC, false);
        return x + w + 4;
    }

    private int button(GuiGraphics ctx, int x, int y, String label, int accent) {
        int w = this.font.width(label) + 16;
        boolean hov = in(x, y, w, 18);
        roundRect(ctx, x, y, w, 18, hov ? mix(C_INNER, accent, 0.35f) : C_INNER);
        ctx.drawString(this.font, Component.literal(label), x + 8, y + 5, 0xFFD0D0DA, false);
        return x + w + 6;
    }
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (in(bx, winY + 30, ow, 18)) {
            net.minecraft.Util.getPlatform().openUri(SITE + "/presets.html");
            return true;
        }
