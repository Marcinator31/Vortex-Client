...
public final class HudRenderer {
    ...
    private static void onHudRender(DrawContext context) {
        ...
        try {
            ...
            com.example.pvpclient.core.Profiler.record("HUD", System.nanoTime() - pvpT0);
        }
    }
    ...
    private static void onHudRenderInner(DrawContext context) {
        ...
        com.example.pvpclient.hud.WaypointHud.draw(context, client);
    ...
    private static void drawTotemPopper(DrawContext ctx, MinecraftClient client) {
        ...
        var list = com.example.pvpclient.hud.TotemPops.top(mod.maxEntries.getInt());
    ...
    private static void drawSessionStats(DrawContext ctx, MinecraftClient client) {
        ...
        if (mod.showTime.get()) {
            ctx.drawTextWithShadow(client.textRenderer,
                    Text.literal("Zeit: " + com.example.pvpclient.hud.SessionStats.playtime()),
                    bx, ly, col);
            ly += 10;
        }
        if (mod.showDeaths.get()) {
            ctx.drawTextWithShadow(client.textRenderer,
                    Text.literal("Tode: " + com.example.pvpclient.hud.SessionStats.getDeaths()),
                    bx, ly, col);
            ly += 10;
        }
        if (mod.showTotems.get()) {
            ctx.drawTextWithShadow(client.textRenderer,
                    Text.literal("Totems: " + com.example.pvpclient.hud.SessionStats.getOwnTotems()),
                    bx, ly, col);
            ly += 10;
        }
        if (mod.showMaxCps.get()) {
            ctx.drawTextWithShadow(client.textRenderer,
                    Text.literal("Max CPS: " + com.example.pvpclient.hud.SessionStats.getMaxCps()),
                    bx, ly, col);
        }
        popScale(ctx);
    }
