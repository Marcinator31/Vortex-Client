...
public final class WorldScan {
    ...
    public static void register() {
        ClientTickEvents.END_CLIENT_TICK.register(client -> {
            ...
            try {
                ...
                com.example.pvpclient.core.Errors.report("WorldScan", pvpErr);
            } finally {
                com.example.pvpclient.core.Profiler.record("WorldScan",
                        System.nanoTime() - t0);
            }
        });
    }
    ...
    private static boolean anyConsumerActive() {
        var mm = com.example.pvpclient.module.ModuleManager.INSTANCE;
        var cont = mm.get(com.example.pvpclient.module.modules.ContainerEspModule.class);
        if (cont != null && cont.isEnabled()) return true;
        var spawn = mm.get(com.example.pvpclient.module.modules.SpawnerEspModule.class);
        if (spawn != null && spawn.isEnabled()) return true;
        var stash = mm.get(com.example.pvpclient.module.modules.StashFinderModule.class);
        if (stash != null && stash.isEnabled()) return true;
        var sus = mm.get(com.example.pvpclient.module.modules.SusChunksModule.class);
