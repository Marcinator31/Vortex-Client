...
import com.example.pvpclient.skin.SkinFetcher;
import com.example.pvpclient.skin.SkinTextureCache;
import com.example.pvpclient.skin.SkinWardrobe;
...
public class SkinScreen extends Screen {
    ...
    private void drawGrid(DrawContext ctx, int accent) {
        ...
        for (SkinWardrobe.Skin skin : list) {
            ...
            boolean isActive = com.example.pvpclient.skin.ActiveSkin.get() == skin;
            ...
            boolean can = com.example.pvpclient.skin.SkinUploader.canUpload();
            int ubX = cx + 6;
    ...
    private void drawSkinPreview(DrawContext ctx, SkinWardrobe.Skin skin,
        ...
        try {
            ...
            com.example.pvpclient.core.Errors.report("SkinScreen.preview", pvpErr);
        }
    }
    ...
    private void drawFooter(DrawContext ctx, int accent) {
        ...
        var actNow = com.example.pvpclient.skin.ActiveSkin.get();
        if (actNow != null) {
            String vl = "Variante " + com.example.pvpclient.skin.ActiveSkin.getVariant();
        ...
        if (!status.isEmpty()) {
            ...
            var act = com.example.pvpclient.skin.ActiveSkin.get();
            msg = (act == null)
        ...
        for (int i = hits.size() - 1; i >= 0; i--) {
            ...
            switch (h.act) {
                ...
                case MODEL:
                    ...
                    return true;
                case DELETE:
                    if (com.example.pvpclient.skin.ActiveSkin.get() == h.skin) {
                        com.example.pvpclient.skin.ActiveSkin.clear();
                    }
                    ...
                    return true;
                case VARIANT:
                    com.example.pvpclient.skin.ActiveSkin.toggleVariant();
                    status = "Variante " + com.example.pvpclient.skin.ActiveSkin.getVariant()
                ...
                case PICK:
                    ...
                    if (com.example.pvpclient.skin.ActiveSkin.get() == h.skin) {
                        ...
                        com.example.pvpclient.skin.ActiveSkin.set(h.skin);
    ...
    private void startSearch() {
        ...
        Thread t = new Thread(() -> {
            try {
                ...
                com.example.pvpclient.core.Errors.report("SkinScreen.search", e);
            } finally {
                busy = false;
            }
    ...
    private void startUpload(SkinWardrobe.Skin skin) {
        if (busy) return;
        if (!com.example.pvpclient.skin.SkinUploader.canUpload()) {
        ...
        Thread t = new Thread(() -> {
            try {
                com.example.pvpclient.skin.SkinUploader.upload(skin.path(), skin.slim);
                status = skin.name + " ist jetzt dein Konto-Skin -- fuer alle sichtbar.";
            } catch (Throwable e) {
                String m = e.getMessage();
                status = (m == null || m.isEmpty())
                        ? ("Fehler: " + e.getClass().getSimpleName()) : m;
                com.example.pvpclient.core.Errors.report("SkinScreen.upload", e);
            } finally {
                busy = false;
            }
    ...
    private void openFolder() {
        try {
            ...
            com.example.pvpclient.core.Errors.report("SkinScreen.openFolder", pvpErr);
        }
    }
