...
public class ClickGui extends Screen {
    ...
    private void drawHeader(DrawContext ctx, int x, int y, int w, int accent, Theme t) {
        ...
        int px = x + PAD + 70;
        int cur = com.example.pvpclient.core.ConfigManager.getActivePreset();
        for (int i = 0; i < com.example.pvpclient.core.ConfigManager.PRESET_COUNT; i++) {
    ...
    private void drawSidebar(DrawContext ctx, int x, int y, int h,
        ...
        cy = drawSectionEntry(ctx, x, cy, "Waypoints", Section.WAYPOINTS,
                String.valueOf(com.example.pvpclient.waypoint.WaypointManager.all().size()),
    ...
    private void drawSectionContent(DrawContext ctx, int x, int y, int w, int h,
        ...
        switch (section) {
            case WAYPOINTS: {
                var wp = com.example.pvpclient.waypoint.WaypointSettings.INSTANCE;
                int count = com.example.pvpclient.waypoint.WaypointManager.all().size();
                ...
                for (Setting st : wp.getSettings()) {
                    ...
                    // Klickflaeche fuer diese Einstellung (Modul ist null).
                    cy += SET_H;
                }
                break;
            }
        ...
        contentHeight = 0;   // diese Bereiche scrollen nicht
    }
    ...
    private int drawSubButton(DrawContext ctx, Module m, int cx, int sy, int cw,
        ...
        } else if (m instanceof com.example.pvpclient.module.modules.BlockEspModule) {
            label = "Bloecke auswaehlen"; act = Act.SUB_BLOCK;
        } else if (m instanceof com.example.pvpclient.module.modules.AntiRenderModule) {
        ...
        for (int i = hits.size() - 1; i >= 0; i--) {
            ...
            switch (hit.act) {
                ...
                case PRESET: {
                    ...
                    com.example.pvpclient.core.ConfigManager.switchTo(target);
                    expanded.clear();
                    expandAnim.clear();
                    toggleAnim.clear();
    ...
    private void openColor(Module m, ColorSetting c) {
        if (m != null && m instanceof com.example.pvpclient.module.modules.GlobalHudColorModule ghc
                && c == ghc.color) {
    ...
    private int extraHeight(Module m) {
        int h = 6;
        if (m instanceof com.example.pvpclient.module.modules.EspModule
                || m instanceof com.example.pvpclient.module.modules.BlockEspModule
                || m instanceof com.example.pvpclient.module.modules.AntiRenderModule
                ) {
            h += SUB_H;
        }
        for (Setting s : m.getSettings()) {
    ...
    private void pvpclient$captureKeyIfListening() {
        ...
        if (listening == null) {
            for (Setting s : com.example.pvpclient.waypoint.WaypointSettings
