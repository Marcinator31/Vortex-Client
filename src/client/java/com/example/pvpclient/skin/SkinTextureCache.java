    ...
    public static Identifier get(SkinWardrobe.Skin skin) {
        ...
        try {
            ...
            if (image.getWidth() != 64 || (image.getHeight() != 64 && image.getHeight() != 32)) {
                image.close();
                FAILED.put(key, true);
                com.example.pvpclient.core.Errors.note("SkinTextureCache",
                        skin.fileName + ": ungewoehnliche Groesse, erwartet 64x64");
                return null;
            }

            Identifier id = Identifier.of("pvpclient", "skins/" + safeKey(key));
            MinecraftClient.getInstance().getTextureManager().registerTexture(
                    id, new NativeImageBackedTexture(() -> "pvpclient-skin", image));
            CACHE.put(key, id);
            return id;
        } catch (Throwable pvpErr) {
            FAILED.put(key, true);
            com.example.pvpclient.core.Errors.report("SkinTextureCache.get", pvpErr);
            return null;
        }
    }
